...
_capi_test_one() {
  ...
  if [[ "$tool" == "claude" ]]; then
    ...
    http_code=$(curl -s -o "$tmpbody" -w '%{http_code}' --max-time 8 \
      ...
      -H "content-type: application/json" \
      -d '{"model":"gpt-5.3-codex","input":[{"role":"user","content":"hi"}],"max_output_tokens":1,"stream":true}' 2>/dev/null)
  fi
...
_capi_load_codex() {
  ...
  if [[ "$mode" == "login" ]]; then
    # 登录模式：删除 model_provider 行，让 Codex 回退到官方 OpenAI + OAuth2
    sed -i '' '/^model_provider = /d' "$toml"
  else
    ...
    # 设置 model_provider：替换已有行，或在首个 model = 行后插入
    if grep -q '^model_provider = ' "$toml"; then
      sed -i '' "s/^model_provider = .*/model_provider = \"${active}\"/" "$toml"
    else
      awk -v mp="model_provider = \"${active}\"" '
        /^model = / && !done { print; print mp; done=1; next }
        { print }
